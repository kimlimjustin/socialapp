...
const Post = (params) => {
    ...
    useEffect(() => {
        if(postInfo){
            Axios.get(`http://localhost:5000/comments/get/${postInfo._id}/all`)
            .then(res => {
    ...
    const addComment = (e) => {
        ...
        Axios.post("http://localhost:5000/comments/add", {commenter: userInfo._id, post: postInfo._id, token, comment: inputComment})
        .then(() => {
            Axios.get(`http://localhost:5000/comments/get/${postInfo._id}/all`)
            .then(result => {
